"""Leitura do mart e persistência da flag de interesse.

Não altera raw/mart.lotes: a URL da foto é derivada de leilao_id + lote_id.
"""
...
SCHEMA_SQL = REPO_ROOT / "sql" / "002_lotes_interesse.sql"

# ponytail: listing thumbnail is always img_{lote_id}_1.jpg.
# Ceiling: gallery lives on /lotes/detalhes; scrape that page to get img_N_2+.
...
def apply_schema(engine: Engine) -> None:
    """Cria mart.lotes_interesse se ainda não existir."""
...
def get_leilao_id(engine: Engine, lote_id: int) -> int | None:
    with engine.connect() as conn:
        row = conn.execute(
            text("SELECT leilao_id FROM mart.lotes WHERE lote_id = :lote_id"),
            {"lote_id": lote_id},
        ).fetchone()
